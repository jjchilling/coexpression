...
def process_split_videos(video_folder, audio_folder, output_folder="fused_output", target_fps=6):
    ...
    # Find all _left.mp4 and _right.mp4 videos
    ...
    for video_file in tqdm(sorted(video_files), desc="Processing left/right speakers"):
        ...
        # Extract base audio name (without _left/_right)
        base_name = video_file.replace("_left.mp4", "").replace("_right.mp4", "")
...
if __name__ == "__main__":
    process_split_videos(
        video_folder="/path/to/your/processed_videos",
        audio_folder="/path/to/your/audios",            # original audio files
        output_folder="fused_tensors",                  # fused tensors here
        target_fps=6
    )
